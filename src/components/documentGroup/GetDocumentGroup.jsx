...
function GetDocumentGroup() {
    ...
    const [datas, setDatas] = useState([]); // Store original data
    ...
    const fetchDocumentGroup = async () => {
        try {
            ...
            const response = await customAxios.get(
                `/DocumentGroup/GetList/${showBlocked ? "true" : "false"}`
            );
    ...
    const buildHierarchy = (data) => {
        ...
        data.forEach((item) => {
            if (item.ParentName) {
                ...
                groupMap[item.ParentName]?.children.push(
